...
QUEUE_NAME = os.environ.get("QUEUE_NAME", "appointment.payment.success")
DLX_NAME = os.environ.get("DLX_NAME", EXCHANGE_NAME)
DLQ_ROUTING_KEY = os.environ.get("DLQ_ROUTING_KEY", "appointment.payment.success.dlq")
DLQ_QUEUE = os.environ.get("DLQ_QUEUE", "appointment.payment.success.dlq")

APPOINTMENT_SERVICE_URL = os.environ.get("APPOINTMENT_SERVICE_URL", "http://localhost:5002")
EVENT_STORE_PATH = os.environ.get("EVENT_STORE_PATH", "appointment_payment_success_events.db")
...
class PaymentSuccessConsumer:
    ...
    def handle_event(self, payload: dict):
        ...
        self.logger.info(f"Updating appointment {appointment_id} to PAID")
        status_url = f"{APPOINTMENT_SERVICE_URL.rstrip('/')}/appointments/{appointment_id}/status"
        body = json.dumps({"status": "PAID"}).encode("utf-8")
        headers = {"Content-Type": "application/json"}

        for attempt in range(1, MAX_HTTP_RETRIES + 1):
            try:
                response = self._http_put(status_url, body, headers)
                if 200 <= response.get("status", 500) < 300:
                    self.logger.info("Appointment status updated successfully")
                    return
                if 400 <= response.get("status") < 500:
                    raise FatalException(
                        f"Appointment API returned {response['status']} for appointment {appointment_id}: {response.get('body')}"
                    )
                self.logger.warning(
                    f"Appointment API returned transient status {response['status']}, retrying (attempt {attempt})"
                )
            except RetryableException:
                self.logger.warning(f"Retryable HTTP failure, attempt {attempt}")
            except FatalException:
                raise
            except Exception as exc:
                self.logger.warning(f"HTTP request failed: {exc}, attempt {attempt}")

            if attempt < MAX_HTTP_RETRIES:
                time.sleep(RETRY_DELAY_SECONDS)

        raise RetryableException("Failed to update appointment after retries")

    def _http_put(self, url: str, body: bytes, headers: dict) -> dict:
        request_obj = urllib.request.Request(url, data=body, headers=headers, method="PUT")
        try:
