...
QUEUE_NAME = os.environ.get("QUEUE_NAME", "inventory.payment.success")
DLX_NAME = os.environ.get("DLX_NAME", EXCHANGE_NAME)
DLQ_ROUTING_KEY = os.environ.get("DLQ_ROUTING_KEY", "inventory.payment.success.dlq")
DLQ_QUEUE = os.environ.get("DLQ_QUEUE", "inventory.payment.success.dlq")

INVENTORY_SERVICE_URL = os.environ.get("INVENTORY_SERVICE_URL", "http://localhost:5009")
EVENT_STORE_PATH = os.environ.get("EVENT_STORE_PATH", "inventory_payment_success_events.db")
...
class PaymentSuccessConsumer:
    ...
    def handle_event(self, payload: dict):
        ...
        self.logger.info(f"Fulfilling inventory reservations for appointment {appointment_id}")
        fulfill_url = f"{INVENTORY_SERVICE_URL.rstrip('/')}/inventory/reservations/appointment/{appointment_id}/fulfill"
        body = json.dumps({}).encode("utf-8")
        headers = {"Content-Type": "application/json"}

        for attempt in range(1, MAX_HTTP_RETRIES + 1):
            response = self._http_post(fulfill_url, body, headers)
            status = response.get("status", 500)
            if 200 <= status < 300:
                self.logger.info("Inventory reservations fulfilled successfully")
                return
            if status == 404:
                self.logger.info("No inventory reservations to fulfill, treating as processed")
                return
            if 400 <= status < 500:
                raise FatalException(
                    f"Inventory API returned {status} for appointment {appointment_id}: {response.get('body')}"
                )
            self.logger.warning(f"Inventory API returned transient status {status}, retrying (attempt {attempt})")
            if attempt < MAX_HTTP_RETRIES:
                time.sleep(RETRY_DELAY_SECONDS)

        raise RetryableException("Failed to fulfill inventory reservations after retries")
